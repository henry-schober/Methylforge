...
class RowChecker:
    ...
    def validate_unique_samples(self):
        """
        Assert that the combination of sample name and pod5 filename is unique.

        In addition to the validation, also rename all samples to have a suffix of _T{n}, where n is the
        number of times the same sample exist, but with different pod5 files, e.g., multiple runs per experiment.

        """
        if len(self._seen) != len(self.modified):
            raise AssertionError("The pair of sample name and pod5 must be unique.")
        seen = Counter()
